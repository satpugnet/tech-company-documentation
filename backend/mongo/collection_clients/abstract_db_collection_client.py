...
from mongo.collection_clients.db_operations.db_find_operation import DbFindOperation
from mongo.collection_clients.db_operations.db_insert_operation import DbInsertOperation
...
class AbstractDbCollectionClient(ABC):
    ...
    def _find_all(self):
        return [self.__model_class.from_json(document) for document in DbFindOperation(self.__client, self.__model_class()).find()]

    def _find(self, filter_model):
        return [self.__model_class.from_json(document) for document in DbFindOperation(self.__client, filter_model).find()]

    def _find_one(self, filter_model):
        document = DbFindOperation(self.__client, filter_model).find_one()
        return self.__model_class.from_json(document) if document else None

    def _insert_one(self, insert_model):
        return DbInsertOperation(self.__client, insert_model).insert_one()
