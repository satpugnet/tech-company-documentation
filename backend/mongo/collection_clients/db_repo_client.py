from mongo.collection_clients.abstract_db_collection_client import AbstractDbCollectionClient
from mongo.models.db_repo_model import DbRepoModel


class DbRepoClient(AbstractDbCollectionClient):
    def __init__(self):
        super().__init__('repo', DbRepoModel)

    def upsert(self, github_account_login, repo_name):
        return self._upsert(DbRepoModel(github_account_login=github_account_login, repo_name=repo_name),
                     DbRepoModel(github_account_login=github_account_login, repo_name=repo_name, sha_last_update=""))

    def upsert_sha_last_update_only(self, github_account_login, repo_name, sha_last_update):
        return self._upsert(DbRepoModel(github_account_login=github_account_login, repo_name=repo_name),
                            DbRepoModel(sha_last_update=sha_last_update))

    def find_one(self, repo_name):
        return self._find_one(DbRepoModel(repo_name=repo_name))

    def remove(self, github_account_login):
        return self._remove(DbRepoModel(github_account_login=github_account_login))
