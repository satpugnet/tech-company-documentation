...
class DbGithubFileClient(AbstractDbCollectionClient):
    ...
    def find_one(self, github_account_login, repo_name, dir_path, filename):
        return self._find_one(DbGithubFileModel(github_account_login=github_account_login, repo_name=repo_name, dir_path=dir_path,
                                         filename=filename))

    def upsert(self, github_account_login, repo_name, dir_path, filename, type, content):
        return self._upsert(DbGithubFileModel(github_account_login=github_account_login, repo_name=repo_name,
                                       dir_path=dir_path, filename=filename),
                     DbGithubFileModel(github_account_login=github_account_login, repo_name=repo_name,
                                       dir_path=dir_path, filename=filename, type=type, content=content))

    def remove(self, github_account_login):
        return self._remove(DbGithubFileModel(github_account_login=github_account_login))
